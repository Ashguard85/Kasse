...
const DEFAULT_PAYMENT_SETTINGS = { enabled: { nfc: false, qr: true, bleNfc: true, manual: true }, default: "bleNfc" };
...
function defaultProfileSettings() {
  return { payment: clone(DEFAULT_PAYMENT_SETTINGS), printer: clone(DEFAULT_PRINTER_SETTINGS), receiptLayout: clone(DEFAULT_RECEIPT_LAYOUT_SETTINGS) };
}
...
function customerWithTokens(db,customerId,pid) {
  ...
  return {...clone(c),balance:money(cp?.balance||0),profile_active:Boolean(cp?.active),tokens:clone(db.tokens.filter((t)=>Number(t.customer_id)===Number(customerId)).sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at))))};
}
...
function sanitizePrinterSettings(i={}) { return {enabled:i.enabled===true,address:String(i.address||"").slice(0,120),name:String(i.name||"").slice(0,120)}; }
...
export async function localApiFetch(path,options={}) {
  try {
    ...
    if(pathname==="/api/settings/payment") { if(method==="GET")return jsonResponse(clone(ensureSettings(readDb(),pid).payment)); const body=await parseBody(options); if(!body.enabled||!Object.values(body.enabled).some(Boolean)||!body.enabled[body.default])return jsonResponse({error:"Mindestens eine Methode und ein aktiver Standard sind erforderlich"},400); const r=withDb((db)=>ensureSettings(db,pid).payment=clone(body));return jsonResponse(r); }
    ...
    if(pathname==="/api/customers"&&method==="POST"){const body=await parseBody(options);const bal=nonNegativeNumber(body.balance);if(!body.name||bal===null)return jsonResponse({error:"Name und gültiges Startguthaben sind erforderlich"},400);const r=withDb((db)=>{if((body.nfc_uid&&tokenExists(db,"nfc",body.nfc_uid))||(body.qr_code&&tokenExists(db,"qr",body.qr_code)))return{error:"NFC-UID oder QR-Code wird bereits verwendet",status:409};const c={id:db.nextIds.customer++,name:String(body.name),created_at:nowIso()};db.customers.push(c);ensureCustomerProfile(db,c.id,pid,{active:true,balance:bal});if(body.nfc_uid)db.tokens.push({id:db.nextIds.token++,customer_id:c.id,type:"nfc",value:String(body.nfc_uid).toUpperCase(),active:1,created_at:nowIso()});if(body.qr_code)db.tokens.push({id:db.nextIds.token++,customer_id:c.id,type:"qr",value:String(body.qr_code),active:1,created_at:nowIso()});return customerWithTokens(db,c.id,pid);});return r.error?jsonResponse({error:r.error},r.status):jsonResponse(r);}
    ...
    if(pathname==="/api/checkout"&&method==="POST"){const body=await parseBody(options);const total=positiveNumber(body.total);if(!body.card_uid||total===null)return jsonResponse({error:"card_uid und ein gültiger Betrag (> 0) sind erforderlich"},400);const r=withDb((db)=>{const c=lookupCustomer(db,body.card_uid,pid);if(!c)return{error:"Kunde ist in diesem Profil nicht aktiv oder wurde nicht gefunden",status:404};const cp=ensureCustomerProfile(db,c.id,pid);const rounded=money(total);if(cp.balance<rounded)return{error:"Guthaben zu niedrig",status:402,balance:cp.balance};cp.balance=money(cp.balance-rounded);const items=normalizeCheckoutItems(body);const created=nowIso();const tx={id:db.nextIds.transaction++,profile_id:pid,customer_id:c.id,amount:rounded,type:"purchase",note:`Einkauf: ${items.map((i)=>`${i.article_name} x${i.quantity}`).join(", ")}`,created_at:created};db.transactions.push(tx);items.forEach((i)=>db.saleItems.push({id:db.nextIds.saleItem++,profile_id:pid,transaction_id:tx.id,customer_id:c.id,article_id:i.article_id,article_name:i.article_name,quantity:i.quantity,unit_price:i.unit_price,total:i.total,sold_at:created}));return{success:true,new_balance:cp.balance,customer_name:c.name};});return r.error?jsonResponse({error:r.error,balance:r.balance},r.status):jsonResponse(r);}
    ...
    if(pathname==="/api/status"&&method==="GET"){const db=readDb();return jsonResponse({app:"KinderKasse",version:"2.0.0",profile:clone(findProfile(db,pid)||db.profiles[0])});}
