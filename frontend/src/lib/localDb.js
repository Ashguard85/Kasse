...
export async function localApiFetch(path,options={}) {
  try {
    ...
    if(pathname==="/api/settings/payment") { if(method==="GET"){const current=ensureSettings(readDb(),pid).payment||{};return jsonResponse({...clone(DEFAULT_PAYMENT_SETTINGS),...clone(current),enabled:{...clone(DEFAULT_PAYMENT_SETTINGS.enabled),...(current.enabled||{})}});} const body=await parseBody(options); if(!body.enabled||!Object.values(body.enabled).some(Boolean)||!body.enabled[body.default])return jsonResponse({error:"Mindestens eine Methode und ein aktiver Standard sind erforderlich"},400); const clean={...clone(DEFAULT_PAYMENT_SETTINGS),...clone(body),enabled:{...clone(DEFAULT_PAYMENT_SETTINGS.enabled),...(body.enabled||{})},cashBreakdownEnabled:body.cashBreakdownEnabled===true,customerDisplayEnabled:body.customerDisplayEnabled===true};const r=withDb((db)=>ensureSettings(db,pid).payment=clean);return jsonResponse(r); }
