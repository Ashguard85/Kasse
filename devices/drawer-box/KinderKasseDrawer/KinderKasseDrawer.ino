/*
  KinderKasse Kassenschublade – ESP32-C3 + SG90
  ================================================
  ...
  Board in Arduino IDE:
    - ESP32C3 Dev Module (für ESP32-C3 SuperMini)
    - USB CDC On Boot: Enabled, falls vom Board-Menü angeboten

  SG90 Anschluss:
    - Rot: 5 V externe/stabile Versorgung
    - Braun/Schwarz: GND
    - Orange/Gelb: Signal an SERVO_PIN (Standard GPIO4)
    - ESP32-GND und Servo-GND müssen verbunden sein.
    - Empfohlen: 470–1000 uF Elko nahe am Servo zwischen 5 V und GND.

  Die Winkel werden direkt unten im Code eingestellt:
    SERVO_CLOSED_DEG = Verriegelungs-/Ruhestellung
    SERVO_OPEN_DEG   = Entriegelungsstellung
    OPEN_HOLD_MS     = Zeit in Entriegelungsstellung

  Für den ersten Mechaniktest den Servoarm möglichst noch nicht fest montieren.
  So können die Winkel gefahrlos angepasst werden.

  Betriebsarten:
    - Android lokal: BLE-Gerät "KasseDrawer"
    - Docker: WLAN + /api/drawer/command
